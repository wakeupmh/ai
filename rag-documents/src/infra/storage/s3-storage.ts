...
export default class S3Storage implements StorageInterface {
  ...
  constructor() {
    this.s3Client = AWSXRay.captureAWSv3Client(new S3Client({ region: process.env.AWS_REGION }))
  }
  ...
  public async getObject(storageName: string, key: string): Promise<{ body: Readable; metadata?: any } | undefined> {
    try {
      ...
      if (!Body || !(Body instanceof Readable)) throw new Error('File not found')
